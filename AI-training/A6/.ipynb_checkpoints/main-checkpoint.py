...
*Updated to include first-order differencing for time-series stationarity.*
"""
...
TRAIN_SPLIT = 0.8
BATCH_SIZE = 32
MAX_TUNER_EPOCHS = 20  # Keeping it short for tuning runs
...
def build_tunable_model(hp):
    ...
    model.add(LSTM(units=hp_units_1, return_sequences=True, input_shape=(LOOKBACK_WINDOW, num_features), activation="hard_sigmoid"))
    ...
    model.add(LSTM(units=hp_units_2, return_sequences=False, activation="hard_sigmoid"))
    model.add(Dropout(hp_dropout))
...
def main():
    ...
    # Save original dataframe to reconstruct absolute values later
    original_df = df.copy()
    
    # --- NEW PREPROCESSING STEP ---
    logger.info("Applying first-order differencing to the dataset to enforce stationarity...")
    df = df.diff().dropna()
    # ------------------------------
    ...
    tuner = kt.BayesianOptimization(
        build_tunable_model,
        objective='val_loss',
        max_trials=15,                      # Total model configurations to test
    ...
    # 1. Actual Absolute Values
    actual_abs = original_df.loc[test_dates, 'electrode_V'].values
    
    # 2. Predicted Absolute Values (1-Step Ahead: actual previous value + predicted delta)
    previous_actual_abs = original_df['electrode_V'].shift(1).loc[test_dates].values
    predicted_abs_1step = previous_actual_abs + predictions_real
    
    # 3. Predicted Absolute Values (Cumulative Sum: integrating predicted deltas over time)
    base_value_idx = original_df.index.get_loc(test_dates[0]) - 1
    base_value = original_df['electrode_V'].iloc[base_value_idx]
    predicted_abs_cumsum = base_value + np.cumsum(predictions_real)
    
    # Evaluate Absolute Errors
    mae_abs_1step = mean_absolute_error(actual_abs, predicted_abs_1step)
    mae_abs_cumsum = mean_absolute_error(actual_abs, predicted_abs_cumsum)
    
    # Log everything out cleanly to evaluate if the model is good
    logger.info("==========================================================")
    logger.info("                    FINAL PERFORMANCE METRICS                ")
    logger.info("==========================================================")
    logger.info(f"Root Mean Squared Error (RMSE) : {rmse:.4f}  (Delta V/step)")
    logger.info(f"Mean Absolute Error (MAE)        : {mae:.4f}  (Delta V/step)")
    logger.info(f"Coefficient of Determination(R²): {r2:.4f}      (Goal: Close to 1.0)")
    logger.info("==========================================================")
    
    if r2 > 0.85:
        logger.info("Verdict: Strong Model! The R² score shows high variance explanation on the differenced trends.")
    elif r2 > 0.60:
        logger.info("Verdict: Moderate Model. Captures differenced trends but struggles with precision.")
    else:
        logger.info("Verdict: Weak Model. Prone to time lags or bad offsets in the differences.")

    # --- Export Results to CSV ---
    output_csv_df = pd.DataFrame({
        'time': test_dates,
        'actual_delta': actual_real,
        'predicted_delta': predictions_real,
        'actual_absolute': actual_abs,
        'predicted_abs_1step': predicted_abs_1step,
        'predicted_abs_cumsum': predicted_abs_cumsum
    })
    csv_filename = "iccp_lstm_predictions.csv"
    output_csv_df.to_csv(csv_filename, index=False)
    logger.info(f"Prediction data successfully saved to: '{csv_filename}'")
    # -----------------------------
    ...
    axes[1].plot(test_dates, actual_real, label='Actual Delta Reference Potential', color='#10b981', linewidth=2)
    axes[1].plot(test_dates, predictions_real, label='LSTM Tuned Forecast (Delta)', color='#ef4444', linestyle='--', linewidth=2)
    axes[1].set_title(f"Differenced Actual vs Prediction Comparison (MAE: {mae:.3f})", fontsize=13, fontweight='bold')
    axes[1].set_ylabel("Delta Electrode_V")
    axes[1].set_xlabel("Time")
    axes[1].legend()
    axes[1].grid(True, linestyle='--', alpha=0.5)
    
    axes[2].plot(test_dates, actual_abs, label='Actual Absolute Potential', color='#10b981', linewidth=2)
    axes[2].plot(test_dates, predicted_abs_1step, label=f'Predicted Absolute (1-Step Ahead)', color='#3b82f6', linestyle='--', linewidth=2)
    axes[2].plot(test_dates, predicted_abs_cumsum, label=f'Predicted Absolute (Cumulative Sum)', color='#8b5cf6', linestyle='-.', linewidth=2)
    axes[2].set_title(f"Reconstructed Absolute Potential Comparison (1-Step MAE: {mae_abs_1step:.3f} | CumSum MAE: {mae_abs_cumsum:.3f})", fontsize=13, fontweight='bold')
    axes[2].set_ylabel("Electrode_V")
