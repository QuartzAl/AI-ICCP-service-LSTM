...
def build_tunable_model(hp):
    ...
    # Read global configuration for shape mapping
    num_features = 5 
    
    model = Sequential()
    
    # Tunable Parameter 1: LSTM units for the first structural layer
    hp_units_1 = hp.Int('lstm_units_1', min_value=32, max_value=128, step=32)
    model.add(LSTM(units=hp_units_1, return_sequences=True, input_shape=(LOOKBACK_WINDOW, num_features)))
    
    # Tunable Parameter 2: Dropout rates (your sweet spot was around 0.1)
    hp_dropout = hp.Float('dropout_rate', min_value=0.05, max_value=0.20, step=0.05)
    model.add(Dropout(hp_dropout))
    
    # Tunable Parameter 3: LSTM units for the second sequence layer
    hp_units_2 = hp.Int('lstm_units_2', min_value=16, max_value=64, step=16)
...
def main():
    ...
    tuner = kt.BayesianOptimization(
        build_tunable_model,
        objective='val_loss',
        max_trials=7,                      # Total model configurations to test
    ...
    # 3. Retrieve and isolate best performance models
    ...
    final_early_stopping = EarlyStopping(monitor='val_loss', patience=4, restore_best_weights=True)
    ...
    if r2 > 0.85:
        logger.info("Verdict: Strong Model! The R² score shows high variance explanation.")
    elif r2 > 0.60:
        logger.info("Verdict: Moderate Model. Captures trends but struggles with precision.")
    else:
        logger.info("Verdict: Weak Model. Prone to time lags or bad offsets. Re-examine setup.")
    ...
    axes[1].set_title(f"Actual vs Prediction Comparison (R²: {r2:.3f})", fontsize=13, fontweight='bold')
