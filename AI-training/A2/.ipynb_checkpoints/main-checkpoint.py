...
def query_influxdb_data() -> pd.DataFrame:
    ...
    aggregates data into mean windows, and parses them into a Pandas DataFrame.
    """
...
def main():
    ...
    lr_scheduler = ReduceLROnPlateau(
        monitor='val_loss', 
        factor=0.5,       # Cut the learning rate in half when triggered (e.g., 0.001 -> 0.0005)
        patience=1,        # Wait 2 epochs of no improvement before cutting the rate
        min_lr=1e-6,       # Don't let the learning rate drop below this absolute minimum
        verbose=1          # Prints a message in the console when the rate changes
    )
